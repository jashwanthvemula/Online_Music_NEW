...
import csv
import subprocess
from db_config import DB_CONFIG, APP_CONFIG
...
def hash_password(password):
    """Hash a password or secret key using SHA-256"""
...
def validate_secret_key(email, secret_key):
    """Validate the secret key for a given email"""
    try:
        connection = connect_db()
        if not connection:
            return False
        ...
        cursor.execute(
            "SELECT user_id FROM Users WHERE email = %s AND secret_key = %s",
            (email, hashed_secret_key)
        )
        ...
        print(f"Error validating secret key: {err}")
        return False
    finally:
        if 'connection' in locals() and connection and connection.is_connected():
            cursor.close()
